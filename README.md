...
## NOTICE
<font size="4">**Re-download `MIT-input-path.txt` if the launcher fails to merge & split images when the folder or image names contain non-ASCII characters (e.g. Chinese characters) & special characters (e.g. `'`, `\`, `^`, etc).**</font>

<font size="4">**MIT inpainter can't handle too long images produced by `MIT-local-webtoon-launcher.ps1`. So far the longest images it has successfully inpainted in my tesing were ~122,000 pixels. It fails when I tested in on ~180k px images 🤣**</font>
