...
> - Change the default image merging function back to merge into 1 image instead of 2 (**customizable:** in `settings.json`, change `"merged_image_number": 1`  to another number).
...
> - Add option to specify the number of split parts in `settings.json` (Change "original" in `"split_part_number": "original"` to a number without quotes).
